...
let currentRent = data2[id];
console.log(currentRent)
let nftid = currentRent[1];
let NFTowner =await getNFTownerbyID(nftid);
let requirednft = await setCurrentNFT();
const account =await setAccount();
let done = await contract.methods._viewRentedNftStatus(nftid).send({from:account},);
let nftviews =await contract.methods.rentIdToViews(id).call();
let imagedata = await contract.methods._viewRentedNftStatusoutput(id).call();

console.log (currentRent[0],currentRent[1])
let nftrenter = currentRent[0];
console.log("dataimageis : " + imagedata)
document.getElementById("Imageee").src= await imagedata;
document.getElementById("noviews").innerHTML+= (nftviews).toString();
document.getElementById("OwnerAddress").innerHTML+=  NFTowner.toString();
document.getElementById("renterAddress").innerHTML+= currentRent[0].toString();
if(imagedata=="you have to pay"){
document.getElementById("limit").innerHTML = "Limit reached you have to pay";}
...
async function getName(nftId){  
 ...
 const buttonrent=document.getElementById("pay");
    buttonrent.addEventListener("click",async(event) => {
	event.preventDefault();
	
	await contract.methods.getpaid(nftid).send({from:account,value:0x3A352944000});
